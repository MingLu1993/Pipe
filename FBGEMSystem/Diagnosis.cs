...
namespace FBGEMSystem
{
    class Diagnosis
    {
        CLearn learn = new CLearn();

        //用于故障诊断
        
        public int K = 0;               //进行第k次增量训练
        public MWCellArray FinalTree = new MWCellArray();   //存储所有树结构
        public MWCellArray Finalmacro = new MWCellArray();  //存储所有树的权重
        public MWCellArray FinalMACRO = new MWCellArray();  //存储所有子集成分类器的权重

        public double Rata = 0.7;       //迭代过程中抽取训练样本的比例
        public double Inc_node = 5;     //决策树停止训练的条件，表示样本数小于一定阈值结束递归，可设置为5-10
        public double[] Discrete_dim;   //用于记录每一个特征是否是离散特征，初始化都记为0，代表都是连续特征，
        //public double[,] Trains;        //训练样本，一行为一个样本

        public int T = 0;           //设置每次增量训练的迭代次数，程序初始化时需设置
        public int TargetsNum = 0;  //管路状态个数，类别个数，程序初始化时需设置

        public void TRAIN( double[,] Trains)
        {
            lock(this)
            {
        ...
        public int PREDICT(double[,] test_patterns)
        {
            lock(this)
            {
                if(K>0)
