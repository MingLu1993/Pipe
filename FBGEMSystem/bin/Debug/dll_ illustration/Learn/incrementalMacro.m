function [Subtree,macro,MACRO] = incrementalMacro( k,Subsample,T,rata,inc_node_begain,discrete_dim )
%Subsample  子集
%T          每个子集迭代次数
...
%   Detailed explanation goes here
    ...
    macro=zeros(1,T);     %每次循环的子树的宏平均
    MACRO=zeros(1,T);     %每次循环的集成Ht的宏平均
...
%         fprintf('subsample is %d;t is %d: Epsilon is %f\n',k,t,Epsilon);
        if Epsilon > 1/2
            ...
            macro(t)=ComputeMacro(All_targets,All_targets_predict_t);
            %通过权重投票方式产生集成假设Ht
...
%             end
            ...
            All_targets_predict_vote= SubVote_Macro( t,Subtree(1:t) , All_patterns ,macro(1:t),discrete_dim, TargetsNum);
            Epsilon_jicheng_error_index=find((All_targets_predict_vote~=All_targets)==1);  %查找预测错误的索引
            
            D_tmp=D(Epsilon_jicheng_error_index);                                 %错误的样本的D
            Epsilon_jicheng=sum(D_tmp);
            [~,error]=size(Epsilon_jicheng_error_index);
...
%             fprintf('subsample is %d;t is %d: Epsilon_jicheng is %f\n',k,t,Epsilon_jicheng);
            if Epsilon_jicheng >0.5
                ...
                MACRO(t)= ComputeMacro(All_targets,All_targets_predict_vote);
                ...
                D(Epsilon_jicheng_error_index)= D(Epsilon_jicheng_error_index);
%                 for i=1:AllNum
