function PredictVote = FinalVote_new(K,T, FinalTree,patterns,FinalBeta ,FinalEnsembleBeta , discrete_dim, targetsNum)
    ...
    for k=1:K
        for t=1:T
            PredictVote_tmp(k,t,:) =SubVote_new(t,FinalTree{k},patterns ,FinalBeta{k},discrete_dim, targetsNum);
        end
    end
    ...
    for i=1:AllNum
        ...
        for kk=1:K
            for tt=1:T
                predictVote_array(PredictVote_tmp(kk,tt,i))=predictVote_array(PredictVote_tmp(kk,tt,i))+log(1/FinalEnsembleBeta{kk}(tt));
            end
        end
