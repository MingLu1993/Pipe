...
20170303 MainWindow.xaml.cs开始按钮中，加入对设置的判断，先设置才能开始；
	 开始按钮中，加入是否写入数据库的标志位设置。判断是否对数据库操作。
20170303 增加Learn.dll，为故障诊断动态链接库；
	 加入DIAGNOSIS文件夹，里面为故障诊断增量学习界面；
	 设置T，进行增量训练测试，进行诊断测试；
	 DiagnosisUser.cs类实现增量学习和诊断的函数供其他类调用。